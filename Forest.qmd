...
![Geodist](images/rf_regression_rmse.png){style="width: 20%; float: right; margin-left: 15px; margin-top: 10px; margin-bottom: 10px;"}![rf_var_imp_bins](images/final_var_importance_rf_regress.png){style="width: 20%; float:right; margin-top: 10px; margin-bottom: 10px;"}
...
Comparing these two RF models, we realize that a higher mtry value, consistently maps to a better accuracy and lower error. ![rf_pred](images/random_forest_training_prediction.png){style="width: 20%; float:right; margin-top: 10px; margin-bottom: 10px;"}Mtry stands for the number of random selection of variables per each split of the trees in the random forest, and it makes sense that including a greater variety of possible split variables allows for chosing the optimal varibale to split, reducing the error. On the other hand, we see no pattern for the optimal value for num_trees variable. While the classification task preferred trees = 300, the regression task preferred trees = 150. We argue that this makes sense, because our dataset isn't a big dataset, containing only 107 rows as aforementioned, so a higher number of trees doesn't necessarily introduce more learning ofunderlying patterns. 


- **Model 5:** KNN
![Geodist](images/knn_accuracy.png){style="width: 20%; float: right; margin-left: 15px; margin-top: 10px; margin-bottom: 10px;"}
Lastly, we trained a knn model on our dataset. We used a cross validation approach, utilizing 5 vfolds, and tuning an ideal k value among a list [1,3,5]. We find that the greatest accuracy is achieved by a knn with k = 5, having a mean 0.175 accuracy, predicting class among 10 binned classes. 
:::
