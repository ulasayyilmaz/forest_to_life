...
for (country in unique(joined_data$country)) {
  country_data <- joined_data %>% filter(country == !!country)  # Filter data for the country
  
  # Create a plot for each bird order
  p <- ggplot(country_data, aes(x = year, y = individualCount, color = order, group = order)) +
    geom_line(size = 1.2) +
    geom_point(size = 3) +
    facet_wrap(~order, scales = "free_y") +
    labs(
      title = paste("Bird Order Counts Over Time in", country),
      x = "Year",
      y = "Count of Bird Order"
    ) +
    theme_minimal() +
    theme(legend.position = "bottom")
  
  # Save the plot as a file for each country
  ggsave(
    filename = paste0("plots/", country, "_bird_order_counts.png"),
    plot = p,
    width = 10,
    height = 6
  )
...
# Loop through each country and bird order
for (country in unique(joined_data$country)) {
  for (bird_order in unique(joined_data$order)) {
    # Filter data for the country and bird order
    country_order_data <- joined_data %>%
      filter(country == !!country, order == !!bird_order)
    
    # Generate a heatmap for each year
    p <- ggplot(country_order_data, aes(x = decimalLongitude, y = decimalLatitude)) +
      geom_tile(aes(fill = individualCount), alpha = 0.8) +
      scale_fill_viridis_c(name = "Count") +
      facet_wrap(~year) +
      labs(
        title = paste(bird_order, "Concentration Heatmap in", country),
        x = "Longitude",
        y = "Latitude"
      ) +
      theme_minimal()
    
    # Save the plot as a file for each country and bird order
    ggsave(
      filename = paste0("plots/", country, "_", bird_order, "_heatmap.png"),
      plot = p,
      width = 10,
      height = 6
    )
