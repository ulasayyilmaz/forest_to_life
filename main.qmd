...
```{r}
# Replace NA values in individual_count with 0
joined_data1$individualCount <- ifelse(is.na(joined_data1$individualCount), 0, joined_data1$individualCount)
# Ensure it is a double type
joined_data1$individualCount <- as.double(joined_data1$individualCount)
...
required_years <- c(2000, 2010, 2020)

orders <- unique(joined_data1$order)

# Iterate through each unique combination of order and country
for (cur_country in countries) {
  for (cur_order in orders)
  
  # Filter the rows for the current order and country
  subset_data <- joined_data1[
    joined_data1$order == cur_order & joined_data1$country == cur_country, 
  ]
  # Identify missing years
  missing_years <- setdiff(required_years, subset_data$year)
  
  # For each missing year, create a new row with `order_count` set to 0
  if (length(missing_years) > 0) {
      for (year in missing_years) {
        # Create a new row with all columns matching `joined_data1`
        new_row <- subset_data[1, ]  # Use the first row as a template
        new_row$year <- year         # Set the missing year
        new_row$order_count <- 0     # Set `order_count` to 0
        
        # Replace all other values with NA
        new_row <- new_row %>%
          mutate(across(everything(), ~ NA)) %>%
          mutate(order = cur_order, country = cur_country, year = year, order_count = 0)
  ...
  # Add the completed subset to the new data
  new_data <- rbind(new_data, subset_data)
}

# Arrange the new data by order, country, and year for better readability
new_data <- new_data[order(new_data$order, new_data$country, new_data$year), ]
...
split_index <- initial_split(model_data_log_normalized_bins, prop = 0.7)

# Extract training and test sets
train_data_bins <- training(split_index)
test_data_bins <- testing(split_index)
...
split_index_rf <- initial_split(model_data_log_normalized, prop = 0.8)
train_data_rf <- training(split_index)
test_data_rf <- testing(split_index)
...
# Step 4: Fit Model
rf_fit <- rf_wflow |>
  fit(data = train_data_rf)

# Step 5: Predict on Test Data
rf_preds <- predict(rf_fit, new_data = test_data_rf) |>
  bind_cols(test_data_rf)
...
# Split Data
set.seed(123)  # Ensure reproducibility
split_index_rf1 <- initial_split(model_data_log_normalized_bins, prop = 0.8)
train_data_rf1 <- training(split_index_rf1)
test_data_rf1 <- testing(split_index_rf1)
...
# Step 4: Fit Model
rf_fit1 <- rf_wflow1 |>
  fit(data = train_data_rf1)

# Step 5: Predict on Test Data
rf_preds1 <- predict(rf_fit1, new_data = train_data_rf1) |>
  bind_cols(train_data_rf1)
...
# Step 5: Predict on Test Data
rf_preds2 <- predict(rf_fit1, new_data = test_data_rf1) |>
  bind_cols(test_data_rf1)
...
fit_knn <- knn_wflow |> fit(data = train_data)

set.seed(470)
knn_vfold <- vfold_cv(train_data,
                          v = 5, strata = scaled_log_count)
